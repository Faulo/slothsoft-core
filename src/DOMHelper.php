...
final class DOMHelper {
     ...
     *
     * Only namespaces known to this helper are explicitly rewritten. Unknown namespaces are not assigned
     * a configured prefix, because a prefix can be rebound inside a subtree and normalizing unknown
     * namespace/prefix pairs globally can change the document meaning. Libxml may still serialize an
     * unknown namespace with an equivalent in-scope prefix or default namespace declaration.
     *
     * Known namespaces use the standardized prefixes defined by this helper when a prefix is emitted, such as "svg" for
     * {@see self::NS_SVG} and "html" for {@see self::NS_HTML}. Using a known prefix for an unrelated
    ...
    private function normalizeNode(DOMNode $sourceNode, DOMDocument $retDoc, array $nsList): DOMNode {
        ...
        switch ($sourceNode->nodeType) {
            ...
            case XML_ELEMENT_NODE:
                $tagName = $this->normalizeQualifiedName($sourceNode, $nsList, $sourceNode->parentNode);
                $retNode = $retDoc->createElementNS($sourceNode->namespaceURI, $tagName);
                foreach ($sourceNode->attributes as $childNode) {
                    $tagName = $this->normalizeQualifiedName($childNode, $nsList);
